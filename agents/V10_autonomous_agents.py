'''V09版本的agent实现内容：(基于V09)
        1.idle工具:做完任务子agent主动进入空闲状态,同时进入轮询检测是否有空闲任务
        2.claim_task工具:子agent自动认领空闲任务
        ...
        对应关系：bash---run_bash()
                ...
                shutdown_request---handle_shutdown_request() # 逻辑修改了，主agent要求关闭是 不管子agent是否同意，直接关闭
                plan_approval---handle_plan_review()
                idle---任务结束后标记为空闲，子agent_loop进入轮询状态
